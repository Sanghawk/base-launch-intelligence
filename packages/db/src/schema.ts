...
export const marketSnapshots = pgTable('market_snapshots', {
  ...
  rawPayloadId: uuid('raw_payload_id').references(() => sourceObservations.id),
  createdAt: timestamp('created_at', { withTimezone: true })
    .notNull()
    .defaultNow()
});
...
export const deployerHistorySnapshots = pgTable('deployer_history_snapshots', {
  id: uuid('id').primaryKey().defaultRandom(),
...
export const tokenScores = pgTable('token_scores', {
  id: uuid('id').primaryKey().defaultRandom(),
  tokenId: uuid('token_id')
    .notNull()
    .references(() => tokens.id),
